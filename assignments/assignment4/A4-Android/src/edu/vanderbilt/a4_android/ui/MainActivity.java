...
public class MainActivity extends Activity {
	...
	public void addBalloon(View v) {
		synchronized (mBalloons) {
			...
			// TODO - You fill in here
			BouncyBalloonRunnable runnable = new BouncyBalloonRunnable(balloon,
					mManager);
	...
	private class BouncesLeftTask extends AsyncTask<Void, Void, Integer> {
		...
		protected Integer doInBackground(Void... params) {
			int count = 0;
			synchronized(mBalloons){
				for(BouncyBalloon ball : mBalloons){
					count += ball.getBouncesLeft();
				}
			}
